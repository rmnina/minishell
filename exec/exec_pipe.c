...
/*   Updated: 2023/12/08 17:53:10 by jdufour          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	launch_pipe(t_pipe *pipes, char **envp)
{
	pid_t	pid1;
	pid_t	pid2;
	int		status1;
	...
	{
		pipes->dup_fd = 1;
		process_pipe(pipes->command1, pipes, envp);
	}
	...
	{
		pipes->dup_fd = 0;
		process_pipe(pipes->command2, pipes, envp);
	}
	close(pipes->pipefd[0]);
	close(pipes->pipefd[1]);
...
void	execute_pipe(t_pipe *pipes, char **envp, t_code *code)
{
	...
	pipe(pipes->pipefd);
	pipe_status = launch_pipe(pipes, envp);
