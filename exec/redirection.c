...
void execute_redirection(t_command *command, char **cmd_args, char **envp, t_alloc *garbage) 
{
...
int handle_redirection(t_code *code, int *i, t_command *command, char **envp, t_alloc *garbage)
{
	...
	{
		execute_redirection(command, cmd_args, envp, garbage);
	} else {
		printf("Executing command: %s\n", cmd_args[0]);
	...
	if (command->type == DB_LEFT_CHEV) {
		hd_status = heredoc(command->redirection_file, &pipes, cmd_args, envp, garbage);
		code->code_status = hd_status;
		printf("handle_redirection finished for heredoc\n");
	}
	free_parsed_command_line(cmd_args);
